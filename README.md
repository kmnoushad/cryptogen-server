# NEXIO v6.8 — Realtime BTC Shock Guard + Structural Execution Validation
...
- Cancels an armed setup if it gives back more than 50% of its original impulse wave. A random green bounce after a deep breakdown cannot become a reclaim.
...
- Requires 15–30 minute price momentum and multi-minute taker buying to remain active at the final liquid reclaim, preventing a lone green candle from triggering after the trend has already faded.
...
4. Deploy with Node 20 or newer. Railway installs the locked `ws` dependency from `package-lock.json`. Upgrading from v6.4–v6.7 requires no new SQL and no required new Railway variables; the v6.8 safety variables have defaults.
