...
const makeDetector = (overrides = {}) => {
  ...
  const detector = new FastMoverDetector({
    ...
    isEventGuarded: overrides.isEventGuarded ?? (() => false),
...
test('event guard suppresses new fast-mover alerts before any confirm flight', async () => {
  let klineCalls = 0;
  const { detector, deps } = makeDetector({ isEventGuarded: () => true });
  deps.binance.klines = async () => { klineCalls++; return risingKlines(); };
  const result = feedHistory(detector, { pump: PUMP });
  assert.equal(result.triggered, true);
  assert.equal(result.suppressed, 'eventGuard');
  await flush();
  assert.equal(deps.sent.length, 0);
  assert.equal(deps.events.length, 0);
  assert.equal(detector.metrics.suppressed.eventGuard, 1);
  assert.equal(detector.metrics.triggers, 0, 'trigger never reaches the confirm pipeline');
  assert.equal(klineCalls, 0, 'no REST confirm burn while guarded');
});
...
test('combined-stream websocket lifecycle: connect, message, alert, reconnect on close', async () => {
  ...
  assert.match(socket.url, /fstream\.binance\.com\/market\/stream\?streams=!miniTicker@arr/);
  socket.emit('open');
...
test('tier-2 event guard suppresses trending alerts before any confirm flight', async () => {
  let klineCalls = 0;
  const guarded = makeDetector({ cfg: TRENDING_CFG, isEventGuarded: () => true });
  guarded.deps.binance.klines = async () => { klineCalls++; return risingKlines(); };
  feedTrending(guarded.detector, { priceAt: RAMP_15M, finalJump: 90_000 });
  await flush();
  assert.equal(guarded.deps.sent.length, 0);
  assert.equal(guarded.deps.events.length, 0);
  assert.ok(guarded.detector.trendingSuppressed.eventGuard > 0);
  assert.equal(guarded.detector.trendingMetrics.trendingTriggers, 0);
  assert.equal(klineCalls, 0, 'no REST confirm burn while guarded');
});
...
test('fast-mover and trending alerts carry the BTC bias tag when injected', () => {
  const btcBias = { btcTag: () => '₿ BTC 15m: <b>DOWN</b> (62%) · 30m: DOWN · ⚠️ against this long' };
  const detector = new FastMoverDetector({
    cfg: { enableFastMoverAlerts: true, enableTrendingMover: true },
    binance: {},
    store: {},
    telegram: {},
    btcBias,
  ...
  const trigger = {
    ...
    move: { pct: 1.2, windowSec: 60, window: '15m' },
    volume: { accel: 4.2 },
  };
  const confirm = { buyRatio: 0.58, spreadBps: 3.5 };
  const fast = detector.alertMessage(trigger, confirm);
  assert.match(fast, /₿ BTC 15m: <b>DOWN<\/b> \(62%\) · 30m: DOWN · ⚠️ against this long\n⚠️ <b>Sudden moves/);
  const trending = detector.trendingAlertMessage(trigger, confirm);
  assert.match(trending, /₿ BTC 15m: <b>DOWN<\/b> \(62%\).*against this long\n⚠️ <b>Steady mover/);
});

test('fast-mover alerts are unchanged with a null bias or the tag disabled', () => {
  const plain = new FastMoverDetector({ cfg: {}, binance: {}, store: {}, telegram: {} });
  const trigger = {
    symbol: 'ETHUSDT',
    price: 3_000,
    quoteVolume24h: 50_000_000,
    move: { pct: 1.2, windowSec: 60 },
    volume: { accel: 4.2 },
  };
  const confirm = { buyRatio: 0.58, spreadBps: 3.5 };
  assert.ok(!plain.alertMessage(trigger, confirm).includes('₿ BTC'));

  const off = new FastMoverDetector({
    cfg: { enableBtcBiasTag: false },
    binance: {},
    store: {},
    telegram: {},
