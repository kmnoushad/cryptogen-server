...
test('accumulated caution blocks entry without becoming terminal candidate risk', () => {
  const risk = assessManipulationRisk({
    features: { ...baseFeatures, extensionAtr: 2.3, upperWickPct: 42 },
    oi,
    depth: { spreadBps: 4, bidNotional05: 300_000, askNotional05: 90_000, imbalance: 1, measuredAt: Date.now() },
    previousDepth: null,
    history,
    fundingPct: 0,
    cfg,
  });
  assert.ok(risk.score >= 5);
  assert.equal(risk.hardBlock, true);
  assert.equal(risk.entryBlocked, true);
  assert.equal(risk.terminalRisk, false);
});

test('catastrophically missing bid depth remains terminal after arming', () => {
  const risk = assessManipulationRisk({
    features: baseFeatures,
    oi,
    depth: { spreadBps: 4, bidNotional05: 20_000, askNotional05: 300_000, imbalance: 0.07, measuredAt: Date.now() },
    previousDepth: null,
    history,
    fundingPct: 0,
    cfg,
  });
  assert.equal(risk.terminalRisk, true);
  assert.equal(risk.hardBlock, true);
});
