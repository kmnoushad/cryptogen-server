...
test('event guard suppresses the alert before the security screen', async () => {
  const h = harness({ eventGuarded: true });
  h.setFeed([rawToken({ price: '1.0' })]);
  await h.radar.pollOnce();
  h.advance(10 * 60_000);
  h.setFeed([rawToken({ price: '1.04' })]);
  ...
  assert.equal(h.messages.length, 0);
  assert.equal(h.events.length, 0);
  assert.equal(h.radar.metrics.suppressed.eventGuard, 1);
  assert.equal(h.radar.metrics.triggers, 0, 'trigger never reaches the screening pipeline');
  assert.deepEqual(h.securityCalls, []); // no wasted on-chain call while guarded
});
...
test('alpha fast-mover alert carries the BTC bias tag when injected', () => {
  ...
  const mover = new AlphaFastMover({ cfg: {}, store: {}, telegram: {}, btcBias });
  const token = {
    symbol: 'APT', chainName: 'BSC', price: 5.5, liquidity: 400_000,
    volume24h: 900_000, holders: 2_500, change24h: 12.5,
  };
  const message = mover.alertMessage(token, { pct: 4.2, windowMin: 10 }, { riskScore: 2, rating: 'OK' });
  assert.match(message, /₿ BTC 15m: <b>DOWN<\/b> \(62%\) · 30m: DOWN · ⚠️ against this long\n⚠️ <b>Early radar ping/);
});

test('alpha fast-mover alert is unchanged with a null bias or the tag disabled', () => {
  const token = {
    symbol: 'APT', chainName: 'BSC', price: 5.5, liquidity: 400_000,
    volume24h: 900_000, holders: 2_500, change24h: 12.5,
  };
  const plain = new AlphaFastMover({ cfg: {}, store: {}, telegram: {} });
  assert.ok(!plain.alertMessage(token, { pct: 4.2, windowMin: 10 }, { riskScore: 2 }).includes('₿ BTC'));
  const off = new AlphaFastMover({
    cfg: { enableBtcBiasTag: false },
    store: {},
    telegram: {},
    btcBias: { btcTag: () => '₿ BTC 15m: <b>UP</b> (70%)' },
  });
  assert.ok(!off.alertMessage(token, { pct: 4.2, windowMin: 10 }, { riskScore: 2 }).includes('₿ BTC'));
});
