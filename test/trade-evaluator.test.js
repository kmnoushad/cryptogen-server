...
test('closed-bar momentum fade closes the database trade and preserves profit', () => {
  const result = evaluateTrade(trade, [{
    openTime: 60_001, closeTime: 119_999, open: 100, high: 100.9, low: 100.1, close: 100.2,
  }], cfg);
  assert.equal(result.closed, true);
  assert.equal(result.patch.exit_reason, 'MOMENTUM_FADE');
  assert.equal(result.patch.outcome, 'WIN');
  assert.ok(result.patch.net_pnl_pct > 0);
});
