...
test('steady multi-minute momentum arms without a sudden 3m pump', () => {
  const candles = [];
  let price = 100;
  for (let i = 0; i < 60; i++) {
    const change = i % 4 === 0 ? -0.08 : 0.07;
    const open = price;
    ...
    candles.push({
      ...
      close,
      volume: 100,
      quoteVolume: i >= 57 ? 11_000 : 10_000,
      trades: 100,
      takerBuyVolume: 54,
      takerBuyQuoteVolume: 5_400,
    });
    price = close;
  }
  const features = buildFeatures(candles);
  assert.equal(features.fastImpulse, false);
  assert.equal(features.steadyImpulse, true);
  assert.equal(features.impulse, true);
  assert.equal(features.setupType, 'STEADY_MOMENTUM');
});

test('slower liquid trend can arm without satisfying fast or steady thresholds', () => {
  const candles = [];
  let price = 100;
  for (let i = 0; i < 60; i++) {
    const open = price;
    const change = i < 30 ? 0 : i % 2 === 0 ? 0.071 : -0.03;
  ...
  const features = buildFeatures(candles);
  assert.equal(features.fastImpulse, false);
  assert.equal(features.steadyImpulse, false);
  assert.equal(features.liquidTrendProbe, true);
  assert.equal(features.setupType, 'LIQUID_TREND');
});
