...
export class Telegram {
  ...
  signalMessage(trade, btc) {
    ...
    return `🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨\n` +
      ...
      `✅ Closed breakout → retest → reclaim\n` +
  ...
  priorityMessage(rows, btc) {
    if (!rows.length) return `📊 <b>NEXIO PRIORITY LIST</b>\n━━━━━━━━━━━━━━━\nNo setup currently above the quality floor.\n⏰ ${gstTime()} GST`;
    const ranks = ['🥇','🥈','🥉','4️⃣','5️⃣','6️⃣','7️⃣','8️⃣'];
    const body = rows.slice(0, 8).map((row, index) => {
      const ageSec = Math.max(0, Math.floor((Date.now() - row.updatedAt) / 1_000));
      const fresh = ageSec <= 90 ? '🟢' : ageSec <= 240 ? '🟡' : '🔴';
      const state = row.state === 'RETESTED' ? '⚡ RETESTED' : row.state === 'ARMED' ? '🎯 ARMED' : '👀 BUILDING';
      const flags = [
        `${fresh}${ageSec}s`,
        row.quoteVolume ? `💧${usd(row.quoteVolume)}` : '',
        row.buyRatio != null ? `🟢buy ${(row.buyRatio * 100).toFixed(0)}%` : '',
      ].filter(Boolean).join(' · ');
      return `${ranks[index]} 📈 <b>LONG ${escapeHtml(row.symbol.replace('USDT', ''))}</b> — ${state} <b>${row.score.toFixed(1)}/10</b>\n` +
        `${flags}\n${confidenceBar(row.score)}`;
    }).join('\n');
    return `📊 <b>NEXIO PRIORITY LIST</b>\n` +
      `━━━━━━━━━━━━━━━\n${body}\n` +
      `━━━━━━━━━━━━━━━\n` +
      `₿ BTC ${escapeHtml(btc.regime)} ${btc.allowed ? '🟢' : '⛔'} · Closed 1m data\n` +
      `🔥 FIRE only after a valid retest/reclaim\n` +
      `⏰ ${gstTime()} GST · <i>DYOR · SL always set</i>`;
  }

  alphaQualifiedMessage(token, qualification, security) {
    return `🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯\n` +
      `<b>🎯 [ALPHA] QUALIFIED — ${escapeHtml(token.symbol)}</b>\n` +
      `<b>${escapeHtml(token.name)} · ${escapeHtml(token.chainName)}</b>\n` +
      `🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯\n\n` +
      `📊 Setup: <b>${qualification.score}/10</b>\n${confidenceBar(qualification.score)}\n` +
      `💵 Price: $${formatPrice(token.price)} · 24h ${token.change24h >= 0 ? '+' : ''}${token.change24h.toFixed(1)}%\n` +
      `💧 Liquidity ${usd(token.liquidity)} · Volume ${usd(token.volume24h)}\n` +
      `👥 Holders ${Math.round(token.holders).toLocaleString()} · Vol/Liq ${qualification.volLiq.toFixed(1)}x\n` +
      `🔐 Rug check: <b>${alphaRiskLabel(security)}</b> (${security.riskScore}/10)\n` +
      `${securityLines(security)}\n\n` +
      `<i>Monitoring for ignition. Not an entry yet.</i>\n` +
      `⏰ ${gstTime()} GST`;
  }
  ...
  alphaRiskMessage(token, security, reasons, kind) {
    const reasonLines = [...new Set([...(reasons ?? []), ...(security?.critical ?? []), ...(security?.warnings ?? [])])]
      .slice(0, 6).map(item => `• ${escapeHtml(item)}`).join('\n');
    return `🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨\n` +
      `<b>🚨 [ALPHA] POSSIBLE RUG — ${escapeHtml(token.symbol)}</b>\n` +
      `<b>${escapeHtml(kind.replaceAll('_', ' '))} · ${escapeHtml(token.chainName)}</b>\n` +
      `🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨\n\n` +
      `💵 $${formatPrice(token.price)} · 💧 liquidity ${usd(token.liquidity)}\n` +
      `🔐 Status: <b>${alphaRiskLabel(security)}</b>\n` +
      `${reasonLines || '• Critical risk state detected'}\n\n` +
      `<b>⛔ NO NEW ENTRY · EXIT/REDUCE IF HOLDING</b>\n` +
      `<i>Automated warning; verify contract manually.</i>\n` +
      `⏰ ${gstTime()} GST`;
  }
