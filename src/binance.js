...
export const classifyBtcRegime = (hourly, five, {
  ...
  // A strong hourly trend must not be switched off by an ordinary 5m/15m
  // pullback. v6.6 made microTrend mandatory, which produced NO_LONG_EDGE even
  // while BTC sat above a rising EMA50/200. Keep the pullback bounded and let
  // each contract's closed-bar reclaim decide the actual entry.
  const controlledMicroPullback = htfTrend
    && fiveMinuteReturn > -0.50
    && fifteenMinuteReturn > -0.80
    && oneHourReturn > -1.00;
  const shock = fiveMinuteReturn <= -1 || fifteenMinuteReturn <= -1.5 || oneHourReturn <= -2;
  const allowed = !shock && (htfTrend || supportRetest) && (microTrend || controlledMicroPullback);
  const regime = shock ? 'SHOCK_BLOCK'
    : allowed && htfTrend && microTrend ? 'BULLISH'
    : allowed && htfTrend ? 'BULLISH_PULLBACK'
