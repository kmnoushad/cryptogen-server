...
export class Engine {
  constructor({ cfg, binance, store, telegram, alpha = null, calendar = null, realtimeShock = null, fastMover = null }) {
    this.cfg = cfg;
  ...
  fastMoverStatusLine() {
    ...
    return `Fast mover: ${state} · ${health.trackedSymbols} tracked · ${health.metrics?.alerts ?? 0} alerts`;
  }
  ...
  async command(message) {
    ...
    if (text === '/start' || text === '/help') {
      await this.telegram.send(`🧪 <b>NEXIO v${APP_VERSION} Actionable Alerts</b>\n` +
        '/version /status /why /diagnostics /audit /stats /events /scan /alphascan /pause /resume /help');
