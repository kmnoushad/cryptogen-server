...
export class Engine {
  constructor({ cfg, binance, store, telegram, alpha = null, calendar = null, realtimeShock = null }) {
    this.cfg = cfg;
  ...
  async handleRealtimeShock(event) {
    const cancelled = this.candidates.size;
    if (cancelled) {
  ...
  async context(symbol, features) {
    ...
    depth.bidRetention = previousDepth?.bidNotional05 > 0
      ? depth.bidNotional05 / previousDepth.bidNotional05
      : null;
    depth.spreadExpansion = previousDepth?.spreadBps > 0
      ? depth.spreadBps / previousDepth.spreadBps
      : null;
  ...
  async command(message) {
    ...
    if (text === '/start' || text === '/help') {
      ...
      const dayLines = dayTop.length
        ...
        : 'Waiting for the first completed 15-minute v6.8 window.';
