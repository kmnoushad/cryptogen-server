...
export const buildFeatures = candles => {
  ...
  const flow15 = takerFlow(last15);
  const atr14 = atr(candles, 14);
  const ema20 = ema(candles.map(c => c.close), 20);
  const ema20FiveBarsAgo = ema(candles.slice(0, -5).map(c => c.close), 20);
  if (!(atr14 > 0) || !(ema20 > 0) || !(ema20FiveBarsAgo > 0) || !(baselineMedian > 0)) return null;
  ...
  const features = {
    ...
    ret15m: pctChange(candles.at(-16).close, last.close),
    ret30m: pctChange(candles.at(-31).close, last.close),
    breakoutLevel,
    breakoutPct: pctChange(breakoutLevel, last.close),
    breakoutGapAtr: (last.close - breakoutLevel) / atr14,
    recentPeak,
    ...
    buyRatio15: flow15.buyRatio,
    deltaRatio1: flow1.deltaRatio,
    deltaRatio3: flow3.deltaRatio,
    deltaRatio15: flow15.deltaRatio,
    ema20Slope5Pct: pctChange(ema20FiveBarsAgo, ema20),
    ...shape,
  };

  features.fastImpulse = features.ret3m >= 0.6
    && features.ret3m <= 2.6
    && features.ret5m <= 3.4
  ...
  // A second, slower path catches persistent movers that climb without a
  // single +0.6% three-minute burst. It still requires a near-breakout price,
  // sustained real taker buying, positive EMA slope and non-climax extension.
  features.steadyImpulse = features.ret15m >= 0.45
    && features.ret15m <= 3.5
    && features.ret30m >= 0.75
    && features.ret30m <= 6.0
    && features.ret5m >= -0.35
    && features.breakoutGapAtr >= -0.35
    && features.impulseVolumeRatio >= 1.0
    && features.buyRatio15 >= 0.525
    && features.ema20Slope5Pct >= 0.04
    && features.last.close > features.ema20
    && features.upperWickPct <= 45
    && features.extensionAtr <= 1.8
    && features.rsi <= 80;

  // Deep, established perpetuals usually move more slowly than low-float
  // contracts. This is only a probe: the engine must subsequently verify the
  // full configured $ depth on both sides, spread, OI and manipulation risk
  // before it can arm. Final retest/reclaim execution rules are unchanged.
  features.liquidTrendProbe = features.ret15m >= 0.25
    && features.ret15m <= 2.5
    && features.ret30m >= 0.45
    && features.ret30m <= 4.0
    && features.ret5m >= -0.20
    && features.breakoutGapAtr >= -0.60
    && features.impulseVolumeRatio >= 0.90
    && features.buyRatio15 >= 0.53
    && features.ema20Slope5Pct >= 0.02
    && features.last.close > features.ema20
    && features.upperWickPct <= 45
    && features.extensionAtr <= 1.60
    && features.rsi <= 76;

  features.impulse = features.fastImpulse || features.steadyImpulse || features.liquidTrendProbe;
  features.setupType = features.fastImpulse ? 'FAST_BREAKOUT'
    : features.steadyImpulse ? 'STEADY_MOMENTUM'
    : features.liquidTrendProbe ? 'LIQUID_TREND'
    : null;
  ...
  const steadyScore = clamp(
    (features.ret15m >= 1.2 ? 2 : features.ret15m >= 0.45 ? 1 : 0)
    + (features.ret30m >= 2 ? 2 : features.ret30m >= 0.75 ? 1 : 0)
    + (features.buyRatio15 >= 0.56 ? 2 : features.buyRatio15 >= 0.525 ? 1 : 0)
    + (features.impulseVolumeRatio >= 1.30 ? 2 : features.impulseVolumeRatio >= 1.0 ? 1 : 0)
    + (features.ema20Slope5Pct >= 0.12 ? 1 : features.ema20Slope5Pct >= 0.04 ? 0.5 : 0)
    + (features.extensionAtr <= 1.4 ? 1 : features.extensionAtr <= 1.8 ? 0.5 : 0),
    0, 10,
  );
  const liquidScore = clamp(
    (features.ret15m >= 0.60 ? 2 : features.ret15m >= 0.25 ? 1 : 0)
    + (features.ret30m >= 1.0 ? 2 : features.ret30m >= 0.45 ? 1 : 0)
    + (features.buyRatio15 >= 0.56 ? 2 : features.buyRatio15 >= 0.53 ? 1 : 0)
    + (features.impulseVolumeRatio >= 1.25 ? 2 : features.impulseVolumeRatio >= 0.90 ? 1 : 0)
    + (features.ema20Slope5Pct >= 0.08 ? 1 : 0.5)
    + (features.extensionAtr <= 1.20 ? 1 : 0.5),
    0, 10,
  );
  features.setupScore = features.setupType === 'STEADY_MOMENTUM' ? steadyScore
    : features.setupType === 'LIQUID_TREND' ? liquidScore
    : fastScore;
...
export const impulseBlockers = features => {
  ...
  ].filter(Boolean).map(reason => `FAST: ${reason}`);
  const steady = [
    features.ret15m < 0.45 && '15m move below 0.45%',
    features.ret15m > 3.5 && '15m move above 3.50%',
    features.ret30m < 0.75 && '30m move below 0.75%',
    features.ret30m > 6.0 && '30m move above 6.00%',
    features.ret5m < -0.35 && '5m momentum reversed',
    features.breakoutGapAtr < -0.35 && 'more than 0.35 ATR below 21m high',
    features.impulseVolumeRatio < 1.0 && 'volume below baseline',
    features.buyRatio15 < 0.525 && '15m taker buying below 52.5%',
    features.ema20Slope5Pct < 0.04 && 'EMA20 slope below 0.04%',
    features.last.close <= features.ema20 && 'price below EMA20',
    features.upperWickPct > 45 && 'upper wick above 45%',
    features.extensionAtr > 1.8 && 'extension above 1.80 ATR',
    features.rsi > 80 && 'RSI above 80',
  ].filter(Boolean).map(reason => `STEADY: ${reason}`);
  const liquid = [
    features.ret15m < 0.25 && '15m move below 0.25%',
    features.ret15m > 2.5 && '15m move above 2.50%',
    features.ret30m < 0.45 && '30m move below 0.45%',
    features.ret30m > 4.0 && '30m move above 4.00%',
    features.ret5m < -0.20 && '5m momentum reversed',
    features.breakoutGapAtr < -0.60 && 'more than 0.60 ATR below 21m high',
    features.impulseVolumeRatio < 0.90 && 'volume below 0.90x baseline',
    features.buyRatio15 < 0.53 && '15m taker buying below 53%',
    features.ema20Slope5Pct < 0.02 && 'EMA20 slope below 0.02%',
    features.last.close <= features.ema20 && 'price below EMA20',
    features.upperWickPct > 45 && 'upper wick above 45%',
    features.extensionAtr > 1.60 && 'extension above 1.60 ATR',
    features.rsi > 76 && 'RSI above 76',
  ].filter(Boolean).map(reason => `LIQUID: ${reason}`);
  return [...fast, ...steady, ...liquid];
};
