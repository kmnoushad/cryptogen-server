...
export class AlphaFastMover {
  constructor({
    ...
    this.metrics = {
      ...
      errors: 0,
      dedupSkipped: 0,
      suppressed: { cooldown: 0, cap: 0, paused: 0 },
    };
  }
  ...
  evaluate(token, key, buffer, nowMs) {
    ...
    if (!hit10 && !hit30) return null; // insufficient history ⇒ no reference ⇒ no trigger
    const move = hit10 ? { pct: move10, windowMin: 10 } : { pct: move30, windowMin: 30 };
