...
export class AlphaRadar {
  ...
  async processToken(token, now, { seedOnly = false, entryEligible = true } = {}) {
    ...
    if (state.state === 'QUALIFIED') {
      ...
      // v6.9.9 Row 6 fix: state used to flip to IGNITED BEFORE emitIgnition
      // confirmed the trade row was actually created — so a persistence
      // failure left the in-memory state believing a trade existed that the
      // database never recorded. Now the state only advances on confirmed
      // success; a false/failed emit leaves the token QUALIFIED so the next
      // poll can retry ignition instead of the token going silently stuck.
