...
export class AlphaRadar {
  ...
  async riskWarning(token, state, security, reasons, kind = 'ONCHAIN_RISK') {
    ...
    const sent = await this.emit(
      ...
      'ALPHA_RUG_WARNING',
      token,
      { kind, reasons, security, last: state.history.at(-1) },
      this.telegram.alphaRiskMessage(token, security, reasons, kind),
    );
    if (sent) this.metrics.warnings++;
  }
  ...
  async processToken(token, now, { seedOnly = false } = {}) {
    ...
    if (state.state === 'SEEDED' && state.history.length >= 3 && qualification.score >= 7) {
      ...
      const eventKey = `alpha:qualified:${key}`;
      const sent = await this.emit(
        eventKey,
        'ALPHA_QUALIFIED',
        token,
        { qualification, security },
        this.telegram.alphaQualifiedMessage(token, qualification, security),
      );
      if (sent) this.metrics.qualified++;
      return;
    }
