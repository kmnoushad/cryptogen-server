...
const STREAM_URL = 'wss://fstream.binance.com/stream?streams=btcusdt@aggTrade/btcusdt@kline_1m/btcusdt@depth20@1000ms';
  ...
  connect() {
    ...
    try {
      ...
      socket.on('open', () => {
        ...
        this.reconnectAttempts = 0;
        this.lastError = null;
        log('BTC feed stream connected (aggTrade + kline_1m + depth20)');
      });
      socket.on('message', raw => this.handleMessage(raw));
      socket.on('error', error => {
        this.lastError = error.message;
        log(`BTC feed stream error: ${error.message}`);
      });
      socket.on('close', () => {
        if (this.socket !== socket) return; // stale socket must not kill a replacement
        this.socket = null;
        this.connected = false;
