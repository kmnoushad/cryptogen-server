...
export class BtcFeed {
    ...
    try {
      ...
      socket.on('open', () => {
        if (isMarket) {
          ...
          this.depthConnectedAt = this.now();
        }
      ...
      socket.on('close', () => {
        ...
        else { this.depthSocket = null; this.depthConnected = false; }
  ...
  health() {
    // v6.9.9 Row 8 fix: the market socket (aggTrade/kline) and the depth
    // socket (/public, order-book) are now reported as two independent
    // connections. Before this, health.connected only reflected the market
    // socket, so a dead depth feed could go unnoticed while the bot appeared
    // fully connected — book-dependent signals (BTC bias, depth checks) would
    // silently run on stale data.
    const depthStale = this.depthConnected
      && Boolean(this.lastBookAt || this.depthConnectedAt)
      && this.now() - (this.lastBookAt || this.depthConnectedAt) > 30_000;
    return {
