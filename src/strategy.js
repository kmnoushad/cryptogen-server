...
export const advanceCandidate = (candidate, features, context, cfg) => {
  ...
  // Reclaim taker-flow thresholds are configurable (v6.9). The slow liquid path
  // keeps its historical 0.03/0.06 relaxation below the fast path (hardcoded
  // 0.57/0.14 -> liquid 0.54/0.08) so it can confirm continuation on milder
  // flow; floors keep the relaxation sane when an operator lowers the fast-path
  // settings toward the config minimum.
  const minReclaimBuyRatio = Number(cfg.futuresReclaimMinBuyRatio ?? 0.55);
  const minReclaimDeltaRatio = Number(cfg.futuresReclaimMinDeltaRatio ?? 0.10);
  const flowBuyRatioMin = liquidTrend
    ? Math.max(0.50, minReclaimBuyRatio - 0.03)
    : minReclaimBuyRatio;
  const flowDeltaRatioMin = liquidTrend
    ? Math.max(0.06, minReclaimDeltaRatio - 0.06)
    : minReclaimDeltaRatio;
  const flowConfirmed = features.buyRatio1 >= flowBuyRatioMin
    && features.deltaRatio1 >= flowDeltaRatioMin
  ...
  // v6.9.7: configurable extension limit + strong-flow allowance. Diagnostics
  // showed "entry extended" blocking 3,425 reclaim attempts during a strong BTC
  // advance while the limit was hardcoded and therefore untestable. A MODEST
  // widening is granted ONLY when the reclaim is backed by genuinely strong
  // taker buying and a rising trend. Every other gate is unchanged.
  const baseExtensionLimit = liquidTrend
    ? Number(cfg.maxEntryExtensionAtrLiquid ?? 1.50)
    : Number(cfg.maxEntryExtensionAtr ?? 1.60);
  const strongFlowContinuation = features.buyRatio1 >= Number(cfg.strongFlowBuyRatio ?? 0.62)
    && Number(features.buyRatio3 ?? 0) >= 0.56
    && Number(features.ema20Slope5Pct ?? 0) >= 0.02
    && last.close > Number(features.ema20 ?? Infinity);
  const extensionLimit = strongFlowContinuation
    ? baseExtensionLimit + Number(cfg.strongFlowExtensionBonusAtr ?? 0.35)
    : baseExtensionLimit;
  const notExtended = features.extensionAtr <= extensionLimit
    && last.close <= candidate.peakPrice + (strongFlowContinuation ? 0.55 : 0.35) * features.atr;
  ...
  const executionHealthy = context.depth.spreadBps <= cfg.maxSpreadBps
    ...
    && depthImbalance >= (cfg.minEntryDepthImbalance ?? 0.60)
