...
export const loadConfig = (env = process.env) => {
  ...
  const cfg = {
    ...
    futuresReclaimMinBuyRatio: numberFrom(env, 'FUTURES_RECLAIM_MIN_BUY_RATIO', 0.55, { min: 0.50, max: 0.70 }),
    futuresReclaimMinDeltaRatio: numberFrom(env, 'FUTURES_RECLAIM_MIN_DELTA_RATIO', 0.10, { min: 0, max: 0.30 }),
    btcBlockHeartbeatMin: numberFrom(env, 'BTC_BLOCK_HEARTBEAT_MIN', 120, { min: 0, max: 1_440 }),
    ...
    enableFastMoverAlerts: boolFrom(env, 'ENABLE_FAST_MOVER_ALERTS', true),
    fastMoverMin1mPct: numberFrom(env, 'FAST_MOVER_MIN_1M_PCT', 0.8, { min: 0.1, max: 10 }),
    fastMoverMin3mPct: numberFrom(env, 'FAST_MOVER_MIN_3M_PCT', 1.5, { min: 0.2, max: 20 }),
    fastMoverVolumeAccel: numberFrom(env, 'FAST_MOVER_VOLUME_ACCEL', 3.0, { min: 1, max: 20 }),
    fastMoverMinQuoteUsd: numberFrom(env, 'FAST_MOVER_MIN_QUOTE_USD', 10_000_000, { min: 100_000 }),
    fastMoverCooldownMin: numberFrom(env, 'FAST_MOVER_COOLDOWN_MIN', 30, { min: 1, max: 1_440 }),
    fastMoverMaxAlertsPerHour: numberFrom(env, 'FAST_MOVER_MAX_ALERTS_PER_HOUR', 6, { min: 1, max: 60 }),
    fastMoverMaxSpreadBps: numberFrom(env, 'FAST_MOVER_MAX_SPREAD_BPS', 30, { min: 1, max: 200 }),
