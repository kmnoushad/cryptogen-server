// ─────────────────────────────────────────────────────────────────────────────
// v5.51 NETWORK FIX: Force IPv4-first DNS resolution.
...
// ─────────────────────────────────────────────────────────────────────────────
// NEXIO SERVER v5.51 — Elite Recovery Edition + Smart Regime
//
...
const ALERT_COOLDOWN_MS       = 900000; // v5.51: 30→15min — catch second legs
...
const MEME_BLACKLIST = new Set([
  ...
  // v5.51: WLD/GMT/GALA removed — legitimate mid-caps with real utility,
...
// v5.51 R:R RECALIBRATION — driven by 212-trade MFE data:
...
// Break-even WR at 1.2 SL / 1.2 TP1 = 50%, but far more trades now REACH TP1.
const UNIFIED_SL_ATR  = 1.2;  // was 1.8 — losers die by +0.67%, cut them faster
const UNIFIED_TP1_ATR = 1.2;  // was 2.0 — capture the move that actually happens
const UNIFIED_TP2_ATR = 2.5;  // was 3.5 — realistic runner for the winners
...
const alphaWatch = new Map();      // v5.51 — QUALIFIED coins under close monitoring
let lastAlphaTop = [];             // for /alpha command
let lastAlphaFetch = 0;

// Rug-risk floors — Alpha coins are wild; these keep the worst junk out
const ALPHA_MIN_LIQUIDITY = 150000;  // $150k liquidity minimum
const ALPHA_MIN_HOLDERS   = 800;     // 800+ holders
const ALPHA_MIN_VOLUME    = 200000;  // $200k 24h volume
const ALPHA_MOVE_ALERT    = 12;      // flag when +12%+ in 24h with volume

const checkAlphaRadar = async () => {
  // Every 4 minutes max
  if (Date.now() - lastAlphaFetch < 4 * 60000) return;
  const isFirstLoad = alphaSeen.size === 0; // v5.51: cold start — seed silently, no alerts
  lastAlphaFetch = Date.now();
  try {
    const res = await fetchJSON(ALPHA_URL);
    if (!res || !res.success || !Array.isArray(res.data)) {
      log('⚠️ Alpha radar: unexpected response shape');
      return;
    }
    const now = Date.now();
    const scored = [];
    for (const t of res.data) {
      const sym = t.symbol || t.name || t.alphaId;
      if (!sym) continue;
      const price   = parseFloat(t.price);
      const chg24   = parseFloat(t.percentChange24h);
      const vol24   = parseFloat(t.volume24h);
      const liq     = parseFloat(t.liquidity);
      const holders = parseInt(t.holders, 10);
      if (!isFinite(price) || !isFinite(chg24)) continue;
      // Rug-risk filter
      if (!(vol24 >= ALPHA_MIN_VOLUME && liq >= ALPHA_MIN_LIQUIDITY && holders >= ALPHA_MIN_HOLDERS)) continue;
      ...
      // ── v5.51 TWO-STAGE FUNNEL: QUALIFY → MONITOR → IGNITION ─────────────────
      ...
      // ALERT 1: freshly-appeared Alpha token — but NEVER on cold start
      if (isNew && !isFirstLoad && now - (prev?.firstSeen || now) < 1000) {
        await postSignal(
          `🆕 <b>[ALPHA] NEW: ${sym}</b>\n` +
          `💵 $${fmtP(price)} · 24h ${chg24 >= 0 ? '+' : ''}${chg24.toFixed(1)}%\n` +
          `💧 Liq $${(liq/1000).toFixed(0)}k · Vol $${(vol24/1000).toFixed(0)}k · 👥${holders}\n` +
      ...
      // v5.51: generic MOVER alert removed — the health analysis above now
    ...
    scored.sort((a, b) => b.chg24 - a.chg24);
    lastAlphaTop = scored.slice(0, 10);
    // prune tokens gone from the list for 24h+
    for (const [s, v] of alphaSeen) if (now - v.firstSeen > 3*24*3600000 && !scored.find(x => x.sym === s)) alphaSeen.delete(s);
    if (alphaSeen.size > 500) { const oldest = [...alphaSeen.entries()].sort((a,b)=>a[1].firstSeen-b[1].firstSeen).slice(0, alphaSeen.size-500); for (const [s] of oldest) alphaSeen.delete(s); }
  } catch (e) {
    log(`⚠️ Alpha radar error (non-fatal): ${e.message}`);
  }
};
...
// v5.51 W7 FIX: dynamic decimals. Old code rounded everything <$1 to 5 places,
// so sub-penny coins (HMSTR $0.00017) showed identical entry/SL/TP. Now scales
// precision to magnitude — every level is distinct and settable.
const fmtP = p => {
  ...
  return p.toPrecision(4); // ultra-micro caps — 4 significant figures
};
...
// v5.51 — MARKET MOVERS RADAR: snapshot of all tickers each full scan,
...
// v5.51 — BTC range context: where does price sit in its multi-day box?
...
const updateBTCRangeContext = async () => {
  try {
    const kl = await fetchJSON('https://fapi.binance.com/fapi/v1/klines?symbol=BTCUSDT&interval=4h&limit=42'); // ~7 days
    ...
    if (span <= 0) return;
    const position = (close - support) / span; // 0 = at support, 1 = at resistance
    // "In range" = not breaking out: within the box, span is a sane 3-15% band
    ...
    btcRangeCtx = {
      ...
      spanPct: parseFloat(spanPct.toFixed(1)),
    };
  } catch { }
};
...
// ── v5.51 IMPULSE SCORER — fixes the architecture's blind spot ───────────────
...
// ── v5.51 CVD (Cumulative Volume Delta) — order-flow approximation ───────────
...
//   price UP but CVD FALLING = pump on selling pressure = FAKE (the FOGO pattern)
//   price FLAT but CVD RISING = quiet accumulation
// Descriptive of what already happened — not a forecast. Zero extra API calls.
const checkCVD = (klines) => {
  ...
  if (priceMove > 1.0 && cvdFalling) {
    divergence = 'BEARISH';           // price up on net selling = distribution into strength
    tag = '⚠️CVD-div';
  } else if (priceMove < -1.0 && cvdRising) {
    divergence = 'BULLISH';           // price down on net buying = absorption
    tag = '🟢CVD-absorb';
  } else if (Math.abs(priceMove) < 1.0 && cvdRising) {
    divergence = 'ACCUM';             // flat price, buyers accumulating
    tag = '🟢CVD-accum';
  }
...
const fetchJSON = async (url, timeout = 8000) => {
  ...
  try {
    ...
    // v5.51 #1: safe JSON parse — a 200 with an HTML/error body would throw
    // a raw SyntaxError and crash the caller. Parse defensively instead.
    const text = await res.text();
    try {
      return JSON.parse(text);
    } catch {
      throw new Error(`Bad JSON from ${url.split('?')[0].slice(-40)} (${text.slice(0, 40)})`);
    }
  } finally { clearTimeout(timer); }
};

const sb = async (path, options = {}, _retry = true) => {
  try {
    ...
    // v5.51 #10: one retry on transient network throw before giving up
    if (_retry) {
      await sleep(800);
      return sb(path, options, false);
    }
    log(`⚠️ Supabase fetch THREW on ${(path||'').split('?')[0]}: ${e.message} [cause: ${e.cause?.code || e.cause?.message || 'unknown'}]`);
    return null;
  }
};
...
const logPaperTrade = async (signal) => {
  try {
    // v5.51 W1 FIX: prevent duplicate OPEN trades on one symbol. If SURGE then
    // FIRE fire on the same coin, two opens would exist and a later close would
    // patch BOTH (corrupting stats). Guarantee one open per symbol.
    const existingOpen = (await sb(`paper_trades?symbol=eq.${signal.symbol}&status=eq.OPEN&select=id`)) || [];
    if (existingOpen.length > 0) {
      log(`⏭ Paper trade skipped: ${signal.symbol} already has an open trade`);
      return;
    }
  ...
  try {
    ...
    for (const trade of open) {
      ...
      try {
        ...
        if (status !== 'OPEN') {
          // v5.51 MFE: compute how far the trade ACTUALLY travelled in our favour
          ...
          let peakPct = null;
          try {
            await sleep(150); // v5.51 W5: space out MFE fetches — mass-close could burst Binance calls (451/418 risk)
...
const _recentSignals = new Map(); // v5.51: full-text hash → timestamp
const postSignal = async text => {
  // v5.51 DEDUP FIX: v5.51 keyed on the first 80 chars, but FIRE/WATCH messages
  // share an identical banner header — so two DIFFERENT coins could collide and
  // the real second alert got wrongly suppressed. Key on the WHOLE message now:
  // identical full text = true duplicate; different coin = different text = sent.
  let hash = 0;
  for (let i = 0; i < text.length; i++) { hash = ((hash << 5) - hash + text.charCodeAt(i)) | 0; }
  const key = String(hash);
  const now = Date.now();
  if (_recentSignals.has(key) && now - _recentSignals.get(key) < 90000) {
    log(`⏭ Duplicate alert suppressed (exact match)`);
    return;
  }
  _recentSignals.set(key, now);
  if (_recentSignals.size > 200) { for (const [k, t] of _recentSignals) if (now - t > 300000) _recentSignals.delete(k); }
...
const runFullMarketScan = async () => {
  if (_fullScanRunning) { log('⏭ Full scan already running — skip overlap'); return; } // v5.51 #3
  ...
  } catch (err) { log('Full scan error:', err.message); } finally { _fullScanRunning = false; }
};

// ── Scanner 2: Watchlist ──────────────────────────────────────────────────────
let _watchScanRunning = false;
const runWatchlistScan = async () => {
  if (_watchScanRunning) { log('⏭ Watchlist scan already running — skip overlap'); return; } // v5.51 #3
  ...
  try {
    ...
    await updateBTCRangeContext(); // v5.51: refresh BTC range box
    await checkAlphaRadar(); // v5.51: Binance Alpha movers (separate from futures)
    ...
    for (const symbol of symbols) {
      ...
      // v5.51: CVD order-flow check — asymmetric by design.
      // The fake-pump penalty is LARGER than the accumulation boost:
      // avoiding a rug is worth more than catching one extra setup.
      // v5.51: RANGE-LOW boost — BTC in bottom third of its box = LONG near support,
      ...
      if (direction === 'LONG' && btcRangeCtx.inRange && btcRangeCtx.position !== null && btcRangeCtx.position <= 0.33) {
        ...
        log(`🟢 RANGE-LOW: ${symbol} — BTC near support (pos ${btcRangeCtx.position}) LONG +1.0`);
      }
      ...
      // v5.51: IMPULSE BOOST — let coins that are ALREADY MOVING score high enough
      ...
      const layers      = { compression, volume, resistance, fundingLS, trap, absorption, dumpTrap, rsi, maStack: maStack.stack, cvd };
      ...
      // v5.51: MOMENTUM SURGE ALERT — the "hybrid momentum layer"
        ...
        if (surgeMove >= 1.5 && surgeAvg > 0 && surgeVol > surgeAvg * 1.3) {
          ...
          if (canAlert(surgeKey)) {
            ...
            await postSignal(
              ...
              `⏰ ${gstNow()} GST`);
            // v5.51: log as paper trade → SURGE win rate becomes measurable in ~2 weeks
      ...
      if (
        ...
        finalScore >= (7.0 + scoreAdj.adjustment) && // v5.51: raised to 7.0 — no alerts below 7 per user; EARLY was the 34%-WR drain
        !ext.tooExtended &&
      ...
      // v5.51: In confirmed BULLISH, EXCEPTIONAL setups (STRONG candle + score 8+)
      ...
      if (sig) {
        ...
        // v5.0: Breakeven at +0.4% profit — lock in "no loss" early (v5.51: 0.5→0.4)
        const inProfitPct = sig.direction === 'LONG'
          ? ((price - sig.price) / sig.price) * 100
          : ((sig.price - price) / sig.price) * 100;

        // v5.51 PEAK & DECAY TRACKING — driven by MFE data (trades stall +0.67% then die)
        if (sig.peakPct === undefined || inProfitPct > sig.peakPct) {
          sig.peakPct = inProfitPct;
          sig.peakAt = Date.now();
        }
        // Momentum-decay exit: was in profit but hasn't made a new peak in ~2 candles (10min)
        if (inProfitPct >= 0.3 && sig.peakPct >= 0.4 && !sig.decayExitSent
            && (Date.now() - (sig.peakAt || 0)) > 10 * 60000) {
          sig.decayExitSent = true;
          signalPrices.set(symbol, sig);
          await postSignal(`⏱ <b>${symbol.replace('USDT','')} MOMENTUM STALL</b> — no new high in 10min\n📊 Peak +${sig.peakPct.toFixed(2)}%, now +${inProfitPct.toFixed(2)}%\n<i>Move stalled — consider closing before it reverses. Your call.</i>\n⏰ ${gstNow()} GST`);
          log(`⏱ DECAY-EXIT: ${symbol} peak +${sig.peakPct.toFixed(2)}% stalled`);
        }
        // Peak-retrace exit: gave back 0.4%+ from the peak (locks the +1.49% winners reach)
        if (sig.peakPct >= 0.6 && (sig.peakPct - inProfitPct) >= 0.4 && !sig.retraceExitSent) {
          sig.retraceExitSent = true;
          signalPrices.set(symbol, sig);
          await postSignal(`📉 <b>${symbol.replace('USDT','')} PEAK RETRACE</b> — pulling back from +${sig.peakPct.toFixed(2)}%\n📊 Now +${inProfitPct.toFixed(2)}% (gave back ${(sig.peakPct - inProfitPct).toFixed(2)}%)\n<i>Reversing off the high — lock profit now. Your call.</i>\n⏰ ${gstNow()} GST`);
          log(`📉 RETRACE-EXIT: ${symbol} peak +${sig.peakPct.toFixed(2)}% → +${inProfitPct.toFixed(2)}%`);
        }

        if (inProfitPct >= 0.4 && !sig.breakevenEarly) {
          await postSignal(`✅ <b>${symbol.replace('USDT','')} BREAKEVEN</b> — Move SL to entry $${fmtP(sig.price)}\n💰 +0.4% secured · Risk now zero\n⏰ ${gstNow()} GST`);
  ...
  } catch (err) { log('Watchlist error:', err.message); } finally { _watchScanRunning = false; }
};
...
const handleCommand = async msg => {
  ...
  if (text === '/start') {
    ...
    const btc = await checkBTCGate();
    const rc = btcRangeCtx;
    const rangeLine = rc.inRange && rc.position !== null
      ? `\n📦 Range: $${rc.support?.toLocaleString()}–$${rc.resistance?.toLocaleString()} (${rc.spanPct}%) · pos ${(rc.position*100).toFixed(0)}%${rc.position <= 0.33 ? ' 🟢 near support' : rc.position >= 0.67 ? ' 🔴 near resistance' : ' ⚪ mid'}`
      : '\n📦 Range: not boxed (trending/breakout)';
    await tg(chatId, `₿ <b>BTC Gate</b>\n${btc.emoji} $${btc.price?.toLocaleString()}\n24h: ${btc.change > 0?'+':''}${btc.change?.toFixed(2)}% | 1H: ${btc.change1H > 0?'+':''}${btc.change1H?.toFixed(2)}%\nFunding: ${btc.funding?.toFixed(3)}%${rangeLine}\n🚦 ${btc.pass ? '✅ PASS' : '❌ BLOCKED'} — ${btc.reason}\n⏰ ${gstNow()}`);
  }
    ...
    if (!lastAlphaTop.length) {
      ...
      msg += `\n<i>DYOR · tight SL · take profit fast on Alpha</i>`;
      await tg(chatId, msg);
    }
  }
  else if (text === '/movers') {
...
const start = async () => {
  ...
  setInterval(() => checkDailySummary().catch(e => log('dailySummary err:', e.message)), 1800000);

  // v5.4 Anomaly check every hour
  setInterval(() => checkAnomalies().catch(e => log('anomalies err:', e.message)), 3600000);

  // BTC regime check — every 5 min
  await checkBTCRegime();
  setInterval(() => checkBTCRegime().catch(e => log('btcRegime err:', e.message)), 300000);

  // v5.10 BTC Cycle Position check every 15 min (warning-only)
  await checkBTCCycle();
  setInterval(() => checkBTCCycle().catch(e => log('btcCycle err:', e.message)), 900000);
  ...
  setInterval(() => checkBTCEarlyWarning().catch(e => log('btcEarly err:', e.message)), 300000);

  // Paper trade outcome checker — every 10 min
  setInterval(() => checkPaperOutcomes().catch(e => log('paperOutcomes err:', e.message)), 600000);
};
