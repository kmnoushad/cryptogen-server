...
test('reclaim is held when the execution-time order book has weak bid support', () => {
  ...
  const weakBook = {
    ...
    depth: {
      ...context.depth,
      imbalance: 0.55, // below the 0.60 minEntryDepthImbalance default
      entryImpactBps: 2,
...
const reclaimedFastCandidate = {
  symbol: 'FLOWUSDT', state: 'RETESTED', detectedBarClose: 60_000, expiresBarClose: 1_500_000,
  breakoutLevel: 100.8, peakPrice: 102, impulseAvgQuoteVolume: 1_000,
  atrAtDetection: 1, setupScore: 8, setupType: 'FAST_BREAKOUT', retested: true, retestLow: 101,
  retestBarClose: 120_000, retestType: 'STANDARD', barsObserved: 3,
};

const softFlowReclaim = {
  last: { closeTime: 180_000, low: 101.1, high: 101.8, close: 101.7, quoteVolume: 1_200 },
  previous: { high: 101.55 }, atr: 1, ret1m: 0.55, ret3m: 1.2, green: true,
  bodyPct: 70, upperWickPct: 14, buyRatio1: 0.56, buyRatio3: 0.60,
  deltaRatio1: 0.11, quoteVolumeRatio: 1.2, extensionAtr: 1.1,
};

test('v6.9 default reclaim flow thresholds confirm a 0.56/0.11 taker bar', () => {
  // Old hardcoded fast path (0.57/0.14) rejected this bar; the v6.9 defaults
  // (0.55/0.10) confirm it. Terminal gates are untouched by this change.
  const result = advanceCandidate(reclaimedFastCandidate, softFlowReclaim, context, cfg);
  assert.equal(result.action, 'SIGNAL');
});

test('reclaim flow thresholds tighten when the operator raises them', () => {
  const strictCfg = { ...cfg, futuresReclaimMinBuyRatio: 0.65, futuresReclaimMinDeltaRatio: 0.25 };
  const result = advanceCandidate(reclaimedFastCandidate, softFlowReclaim, context, strictCfg);
  assert.equal(result.action, 'HOLD');
  assert.match(result.reason, /taker flow weak/);
});

test('liquid path keeps its 0.03/0.06 relaxation below the configured fast path', () => {
  const liquidContext = { ...context, depth: { ...context.depth, bestAsk: 100.9 } };
  const candidate = {
    symbol: 'LIQFLOWUSDT', state: 'RETESTED', detectedBarClose: 60_000, expiresBarClose: 1_500_000,
    breakoutLevel: 100.5, structureLevel: 101.1, peakPrice: 101.2, impulseAvgQuoteVolume: 1_000,
    atrAtDetection: 1, setupScore: 7, setupType: 'LIQUID_TREND', retested: true,
    retestLow: 100.35, retestBarClose: 120_000, retestType: 'STANDARD', barsObserved: 3,
  };
  // buyRatio1 0.53 / deltaRatio1 0.07: below the fast path (0.55/0.10) but
  // above the relaxed liquid path (0.52/0.06).
  const reclaim = {
    last: { closeTime: 180_000, low: 100.45, high: 101.0, close: 100.9, quoteVolume: 900 },
    previous: { high: 100.8 }, atr: 1, ret1m: 0.2, ret3m: 0.6, ret5m: 0.2, green: true,
    bodyPct: 40, upperWickPct: 18, buyRatio1: 0.53, buyRatio3: 0.54, buyRatio15: 0.53,
    deltaRatio1: 0.07, ema20Slope5Pct: 0.04, ema20: 100.5, quoteVolumeRatio: 0.70, extensionAtr: 1.2,
  };
  const result = advanceCandidate(candidate, reclaim, liquidContext, cfg);
  assert.equal(result.action, 'SIGNAL');
  assert.equal(result.trade.setup.setupType, 'LIQUID_TREND');
