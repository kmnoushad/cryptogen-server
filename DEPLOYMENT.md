...
4. Send `/version`, `/status`, `/events`, `/diagnostics`, then `/scan` to the bot. `/version` must report `v6.6.0`.
...
- New entries stop after two losses in the same Dubai day, at the daily/weekly limits, or when BTC is neither bullish nor inside the controlled bullish EMA50 support-retest regime.
