...
4. Add the variables from `.env.example` to Railway. If `MAX_UNIVERSE` already exists, change it from `40` to `60`. Existing v6.4–v6.7 variables can otherwise remain untouched; v6.8's new variables are optional and use safe defaults.
...
4. Send `/version`, `/status`, `/events`, `/diagnostics`, then `/scan` to the bot. `/version` must report `v6.8.0`. `/status` should show `Realtime BTC: ✅ connected` after the WebSocket connects.
...
- A candidate is cancelled if the original impulse wave retraces beyond 50%, even if a later one-minute candle turns green.
...
- `/diagnostics` shows the most common internal Futures gates plus armed/FIRE counts by setup type, without sending rejected setups as alerts.
