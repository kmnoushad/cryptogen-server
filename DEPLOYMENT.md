...
4. Add the variables from `.env.example` to Railway. If `MAX_UNIVERSE` already exists, change it from `40` to `60`. Existing v6.4–v6.8 variables can otherwise remain untouched; no v6.8.1 variable is required.
...
4. Send `/version`, `/status`, `/events`, `/diagnostics`, then `/scan` to the bot. `/version` must report `v6.8.1`. `/status` should show `Realtime BTC: ✅ connected` after the WebSocket connects.
...
- New entries stop after two losses in the same Dubai day, at the daily/weekly limits, or when BTC is outside the bullish, controlled intraday-pullback, and controlled EMA50-retest regimes.
...
- Futures candidates remain active for 24 minutes and survive the five-minute universe rotation. A valid reclaim may wait three additional closed bars for the execution book to recover.
- Candidate invalidation is setup-specific: fast remains strict; steady and liquid-trend setups require wider ATR-normalized structural failure and two consecutive closed bars. Impulse giveback limits are 50% / 60% / 70% and use closed price rather than a wick.
- A rolling BTC drop of 0.35% in 10 seconds cancels all pending Futures candidates immediately and blocks new entries for two minutes. It does not invent a market exit for an existing paper trade; the database stop and closed-bar monitor remain authoritative.
- At reclaim time, Futures requires the configured $1,000 paper order to fit through the live ask ladder within 8 bps, plus stable aggregated depth and spread. Set `ASSUMED_ORDER_NOTIONAL_USD` to the intended order notional before evaluating results.
- `/diagnostics` shows armed → retested → reclaimed → FIRE, terminal rejection reasons, waiting gates and pre-arm filters separately, without sending rejected setups as alerts.
