...
4. Add the variables from `.env.example` to Railway. If `MAX_UNIVERSE` already exists, change it from `40` to `60`. Existing v6.4–v6.6 variables can otherwise remain untouched.
...
4. Send `/version`, `/status`, `/events`, `/diagnostics`, then `/scan` to the bot. `/version` must report `v6.7.0`.
