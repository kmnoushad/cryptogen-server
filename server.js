// ─────────────────────────────────────────────────────────────────────────────
// v5.41 NETWORK FIX: Force IPv4-first DNS resolution.
...
// ─────────────────────────────────────────────────────────────────────────────
// NEXIO SERVER v5.41 — Elite Recovery Edition + Smart Regime
//
...
const ALERT_COOLDOWN_MS       = 900000; // v5.41: 30→15min — catch second legs
...
const MEME_BLACKLIST = new Set([
  ...
  // v5.41: WLD/GMT/GALA removed — legitimate mid-caps with real utility,
...
// v5.41 — MARKET MOVERS RADAR: snapshot of all tickers each full scan,
...
// v5.41 — BTC range context: where does price sit in its multi-day box?
...
// ── v5.41 IMPULSE SCORER — fixes the architecture's blind spot ───────────────
...
// ── v5.41 CVD (Cumulative Volume Delta) — order-flow approximation ───────────
  ...
  try {
    ...
    for (const trade of open) {
      ...
      try {
        ...
        if (status !== 'OPEN') {
          // v5.41 MFE: compute how far the trade ACTUALLY travelled in our favour
...
const runWatchlistScan = async () => {
  ...
  try {
    ...
    await updateBTCRangeContext(); // v5.41: refresh BTC range box
    ...
    for (const symbol of symbols) {
      ...
      // v5.41: CVD order-flow check — asymmetric by design.
      // The fake-pump penalty is LARGER than the accumulation boost:
      // avoiding a rug is worth more than catching one extra setup.
      // v5.41: RANGE-LOW boost — BTC in bottom third of its box = LONG near support,
      ...
      // v5.41: IMPULSE BOOST — let coins that are ALREADY MOVING score high enough
      ...
      // v5.41: MOMENTUM SURGE ALERT — the "hybrid momentum layer"
        ...
        if (surgeMove >= 1.5 && surgeAvg > 0 && surgeVol > surgeAvg * 1.3) {
          ...
          if (canAlert(surgeKey)) {
            ...
            await postSignal(
              ...
              `⏰ ${gstNow()} GST`);
            // v5.41: log as paper trade → SURGE win rate becomes measurable in ~2 weeks
      ...
      // v5.41: In confirmed BULLISH, EXCEPTIONAL setups (STRONG candle + score 8+)
