// ─────────────────────────────────────────────────────────────────────────────
// v5.48 NETWORK FIX: Force IPv4-first DNS resolution.
...
// ─────────────────────────────────────────────────────────────────────────────
// NEXIO SERVER v5.48 — Elite Recovery Edition + Smart Regime
//
...
const ALERT_COOLDOWN_MS       = 900000; // v5.48: 30→15min — catch second legs
...
const MEME_BLACKLIST = new Set([
  ...
  // v5.48: WLD/GMT/GALA removed — legitimate mid-caps with real utility,
...
// v5.48 R:R RECALIBRATION — driven by 212-trade MFE data:
...
const alphaWatch = new Map();      // v5.48 — QUALIFIED coins under close monitoring
...
const checkAlphaRadar = async () => {
  ...
  const isFirstLoad = alphaSeen.size === 0; // v5.48: cold start — seed silently, no alerts
  ...
  try {
    ...
    for (const t of res.data) {
      ...
      // ── v5.48 TWO-STAGE FUNNEL: QUALIFY → MONITOR → IGNITION ─────────────────
      ...
      // v5.48: generic MOVER alert removed — the health analysis above now
...
// v5.48 — MARKET MOVERS RADAR: snapshot of all tickers each full scan,
...
// v5.48 — BTC range context: where does price sit in its multi-day box?
...
// ── v5.48 IMPULSE SCORER — fixes the architecture's blind spot ───────────────
...
// ── v5.48 CVD (Cumulative Volume Delta) — order-flow approximation ───────────
...
const fetchJSON = async (url, timeout = 8000) => {
  ...
  try {
    ...
    // v5.48 #1: safe JSON parse — a 200 with an HTML/error body would throw
...
const sb = async (path, options = {}, _retry = true) => {
  try {
    ...
    // v5.48 #10: one retry on transient network throw before giving up
    if (_retry) {
      await sleep(800);
  ...
  try {
    ...
    for (const trade of open) {
      ...
      try {
        ...
        if (status !== 'OPEN') {
          // v5.48 MFE: compute how far the trade ACTUALLY travelled in our favour
...
const runFullMarketScan = async () => {
  if (_fullScanRunning) { log('⏭ Full scan already running — skip overlap'); return; } // v5.48 #3
...
const runWatchlistScan = async () => {
  if (_watchScanRunning) { log('⏭ Watchlist scan already running — skip overlap'); return; } // v5.48 #3
  ...
  try {
    ...
    await updateBTCRangeContext(); // v5.48: refresh BTC range box
    await checkAlphaRadar(); // v5.48: Binance Alpha movers (separate from futures)
    ...
    for (const symbol of symbols) {
      ...
      // v5.48: CVD order-flow check — asymmetric by design.
      // The fake-pump penalty is LARGER than the accumulation boost:
      // avoiding a rug is worth more than catching one extra setup.
      // v5.48: RANGE-LOW boost — BTC in bottom third of its box = LONG near support,
      ...
      // v5.48: IMPULSE BOOST — let coins that are ALREADY MOVING score high enough
      ...
      // v5.48: MOMENTUM SURGE ALERT — the "hybrid momentum layer"
        ...
        if (surgeMove >= 1.5 && surgeAvg > 0 && surgeVol > surgeAvg * 1.3) {
          ...
          if (canAlert(surgeKey)) {
            ...
            await postSignal(
              ...
              `⏰ ${gstNow()} GST`);
            // v5.48: log as paper trade → SURGE win rate becomes measurable in ~2 weeks
      ...
      if (
        ...
        finalScore >= (7.0 + scoreAdj.adjustment) && // v5.48: raised to 7.0 — no alerts below 7 per user; EARLY was the 34%-WR drain
        !ext.tooExtended &&
      ...
      // v5.48: In confirmed BULLISH, EXCEPTIONAL setups (STRONG candle + score 8+)
      ...
      if (sig) {
        ...
        // v5.0: Breakeven at +0.4% profit — lock in "no loss" early (v5.48: 0.5→0.4)
        const inProfitPct = sig.direction === 'LONG'
          ? ((price - sig.price) / sig.price) * 100
          : ((sig.price - price) / sig.price) * 100;

        // v5.48 PEAK & DECAY TRACKING — driven by MFE data (trades stall +0.67% then die)
