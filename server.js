    ...
    for (const symbol of symbols) {
        ...
        if (surgeMove >= 1.5 && surgeAvg > 0 && surgeVol > surgeAvg * 1.3) {
          ...
          if (canAlert(surgeKey)) {
            ...
            if (!fakeHist.isPumpDump && !dumpTrap.isTrap && !climax.climax && trap.candle?.verdict !== 'FAKE' && !taker.fake && cvd.divergence !== 'BEARISH') {
              ...
              log(`🚫 SURGE-BLOCKED: ${symbol} — failed rug filters`);
            }
          }
        }
      }
      ...
      try {
        ...
        recordCoinBehavior({
          ...
          funding, lsRatio: ls, oi: currentOI, oiChangePct: prevOI > 0 ? parseFloat(((currentOI - prevOI) / prevOI * 100).toFixed(2)) : null,
          rsi: parseFloat(rsi.toFixed(1)), score: parseFloat(finalScore.toFixed(1)), maStack: maStack.stack,
          wasAlerted: false, alertType: null,
        });
      } catch (e) { /* silent */ }

      log(`📊 ${symbol} ${direction} score:${score}${hype.hypeBonus !== 0 ? (hype.hypeBonus > 0 ? '+' : '') + hype.hypeBonus : ''}=${finalScore} candle:${trap.candle?.verdict || 'N/A'} taker:${(taker.ratio*100).toFixed(0)}%`);
      ...
      // ── v5.44: EARLY ENTRY COMPLETELY REMOVED ──────────────────────────────
      // EARLY was 34% WR — it is dead.
      ...
      // ── v5.44: FIRE ALERT — NO ONE-BAR CONFIRMATION DELAY ──────────────────
      // We removed breakoutConfirmed. FIRE now fires on first valid scan.
      const candleOk = trap.candle?.verdict === 'STRONG' || (trap.candle?.verdict === 'WEAK' && finalScore >= 8.5);
      ...
      // v5.44: Instant FIRE — scanCount >= 1 is enough if score is high and candle is strong
      const scanCountOk = state.scanCount >= 1; // was >= 2 or bullConfirmed exception
      ...
      if (block.blocked) {
      ...
      } else if (btc.pass && btcRegime.regime !== 'CHOPPY' && btcSupportive && !pumpCheck.pumped && !inPumpCooldown && !(direction === 'LONG' && climax.climax) && getOpenDirectionCount(direction) < MAX_SAME_DIRECTION && !lowLiq && !dumpTrap.isTrap && !newsEvent && (atrExp.expanding || finalScore >= 7.5) && finalScore >= effectiveMinFire && !shouldBlockFireOnCaution(profile, direction) && scanCountOk && trap.safe && candleOk && !ext.tooExtended && !taker.fake && alertsFired < 2) {
        ...
        if (canAlert(fireKey)) {
          ...
          alertsFired++;
          log(`🚀 FIRED: ${symbol} ${direction} finalScore:${finalScore} (raw:${score}) candle:${trap.candle?.verdict} taker:${(taker.ratio*100).toFixed(0)}%`);
      ...
      // ── Position Manager (unchanged core, compact) ─────────────────────────
      ...
      if (sig) {
        ...
        const milestones = [
          { level: 0.5, key: 'm05', emoji: '💚', msg: 'Close 50% now · Move SL to entry', action: 'PARTIAL_50' },
          { level: 1.0, key: 'm1', emoji: '🟢', msg: 'Close 25% more · Trail rest tight', action: 'PARTIAL_25' },
          { level: 1.5, key: 'm15', emoji: '🚀', msg: 'Close all or trail very tight', action: 'TRAIL' },
          { level: 2.5, key: 'm25', emoji: '💎', msg: 'Runner only — bank the rest', action: 'RUNNER' },
          { level: 4.0, key: 'm4', emoji: '🏆', msg: 'Exceptional — close all, you won', action: 'EXIT' },
        ];
        for (const m of milestones) {
          if (inProfitPct >= m.level && !sig[m.key]) {
            sig[m.key] = true;
            signalPrices.set(symbol, sig);
            await postSignal(`${m.emoji} <b>${symbol.replace('USDT','')} +${m.level}%</b>\n${m.msg}\nCurrent: +${inProfitPct.toFixed(2)}% · Peak: +${(sig.peakPct || inProfitPct).toFixed(2)}%\n⏰ ${gstNow()} GST`);
        ...
        const maxHold = sig.type === 'MOMENTUM' ? 1.5 : 6;
        if (hoursHeld >= maxHold && !sig.timeoutSent) {
          await postSignal(`⏰ <b>${symbol.replace('USDT','')} TIME EXIT</b>\n${maxHold}h held — close position\n📊 Current: ${inProfitPct > 0 ? '+' : ''}${inProfitPct.toFixed(2)}%\n⏰ ${gstNow()} GST`);
          sig.timeoutSent = true;
          if (inProfitPct > 0) { dailyLosses.dailyProfitPct += inProfitPct; recordWin(symbol, inProfitPct); }
          else { dailyLosses.totalPnlPct += inProfitPct; }
          signalPrices.delete(symbol);
          log(`⏰ TIME-EXIT: ${symbol} ${inProfitPct.toFixed(2)}% after ${maxHold}h`);
          try {
            await sb(`paper_trades?symbol=eq.${symbol}&status=eq.OPEN`, {
              method: 'PATCH',
...
// ── Bot Commands (updated /stats, added /movers, rest unchanged) ──────────────
const handleCommand = async msg => {
  ...
  if (text === '/start') {
    ...
    await tg(chatId, `🚫 <b>Meme Coin Blacklist</b>\n━━━━━━━━━━━━━━━\n${MEME_BLACKLIST.size} coins blocked\n\n${list}\n\n<i>Momentum mode can override for parabolic pumps with taker ratio > 60% and score ≥ 7.</i>`);
  }
    ...
    // v5.44: list hot candidates from last full scan (we don't store globally, so explain)
    await tg(chatId, `🚀 <b>Hot Movers</b>\n━━━━━━━━━━━━━━━\nRun /fullscan to refresh the hot momentum radar.\nCoins with 24h +5% to +60% and accelerating 5m candles are auto-added to the watchlist.\n<i>Check /watchlist to see who's heating up.</i>`);
  }
    ...
    if (liveEconomicEvents.length) {
      for (const ev of liveEconomicEvents) {
        if (ev.evTime > now && ev.evTime < now + 14*24*60*60000) upcoming.push({ name: ev.name, evTime: ev.evTime });
      }
    } else {
      for (const ev of ECONOMIC_EVENTS) {
        const evTime = new Date(`${ev.date}T${ev.timeUTC}:00Z`).getTime();
        if (evTime > now && evTime < now + 14*24*60*60000) upcoming.push({ name: ev.name, evTime });
      }
      for (let i = 0; i < 14; i++) {
    ...
    const action = r.regime === 'BULLISH' ? 'LONG only · SHORT blocked' : r.regime === 'BEARISH' ? 'SHORT only · LONG blocked' : '⚠️ ALL signals blocked';
    ...
    const byType = ['FIRE','MOMENTUM','SURGE'].map(t => {
    ...
    await tg(chatId, `📖 <b>Commands</b>\n/start /status /watchlist /tracking /btc /stats /movers /help\n🐆 Nexio v5.44`);
  }

  if (text === '/test') {
    const btc = await checkBTCGate();
    await postSignal(`🧪 <b>NEXIO v5.44 — TEST</b>\n━━━━━━━━━━━━━━━\n✅ Bot online (PAPER MODE)\n✅ 5m Pump Catcher active\n✅ Momentum signals enabled\n✅ Taker Buy Ratio filter active\n✅ Scale-out plan in every alert\n✅ Auto decay/retrace exits\n${btc.emoji} BTC Gate: ${btc.pass?'✅ PASS':'❌ BLOCKED'}\n📊 Watchlist: ${(await getWatchlist()).length}\n🔍 Tracking: ${coinTracker.size}\n⏰ ${gstNow()} GST\n🐆 Nexio v5.44 is watching`);
