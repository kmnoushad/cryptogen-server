// ─────────────────────────────────────────────────────────────────────────────
// NEXIO SERVER v5.0 — COMPLETE REVISION (FIXED)
// 
...
// FIX #6: REGIME UNDEFINED ERROR (scope fix)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// FIX #3: COMPLETE RISK MANAGEMENT SYSTEM
// ─────────────────────────────────────────────────────────────────────────────
const RISK_CONFIG = {
...
account.weekly_date = getWeekStart();
...
// ─────────────────────────────────────────────────────────────────────────────
// FIX #1: DIRECTION ANALYZER (Multi-Timeframe + Order Flow)
// ─────────────────────────────────────────────────────────────────────────────
class DirectionAnalyzer {
  ...
  async analyze(symbol, price, klines, funding, lsRatio) {
    const scores = { long: 0, short: 0, reasons: { long: [], short: [] } };
  ...
  async checkHTFTrend(symbol) {
    try {
      ...
      return { bullish: bullish && makingHigherHighs, bearish: bearish && !makingHigherHighs };
    } catch {
  ...
  async checkOrderFlow(symbol, price) {
    try {
      ...
      for (const bid of ob.bids.slice(0, 50)) bidVolume += parseFloat(bid[0]) * parseFloat(bid[1]);
      for (const ask of ob.asks.slice(0, 50)) askVolume += parseFloat(ask[0]) * parseFloat(ask[1]);
      
      const imbalance = (bidVolume - askVolume) / (bidVolume + askVolume);
      return { bullish: imbalance > 0.2, bearish: imbalance < -0.2, imbalance: parseFloat(imbalance.toFixed(3)) };
    } catch {
      return { bullish: false, bearish: false };
    }
  }
  ...
  checkMomentum(klines) {
    ...
    return parseFloat(((currentPrice - price14ago) / price14ago * 100).toFixed(2));
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// FIX #2: ENTRY TIMING ANALYZER
// ─────────────────────────────────────────────────────────────────────────────
class EntryTimingAnalyzer {
  async analyze(symbol, price, direction, klines, volumeSpike) {
    const signals = { ready: false, type: null, score: 0, waitTime: 0, reasons: [] };
    ...
    if (signals.ready) signals.entryPrice = this.suggestEntryPrice(klines, direction, signals.type);
    return signals;
  }
  ...
  checkBreakoutConfirmation(klines, direction) {
    ...
    return { confirmed: breakoutDirection && confirmationHolds, volumeOk, strength, breakoutPrice: breakClose };
  }
  ...
  checkRetestOpportunity(klines, direction, currentPrice) {
    ...
    if (direction === 'LONG') {
      ...
      const distanceToRetest = ((currentPrice - brokenResistance) / currentPrice) * 100;
      if (distanceToRetest < 1.0 && distanceToRetest > -0.5) {
        return { available: true, level: brokenResistance, estimatedWait: 0 };
      }
    } else {
      const brokenSupport = lows.slice(-20, -10).reduce((a, b) => Math.min(a, b), 0);
      const distanceToRetest = ((brokenSupport - currentPrice) / currentPrice) * 100;
      if (distanceToRetest < 1.0 && distanceToRetest > -0.5) {
        return { available: true, level: brokenSupport, estimatedWait: 0 };
      }
    }
  ...
  checkLiquiditySweepPattern(klines, direction) {
    ...
    if (direction === 'LONG') {
      ...
      const recovered = closes[closes.length - 1] > opens[opens.length - 1] && closes[closes.length - 1] > recentLow;
      if (sweepLow < recentLow * 0.998 && recovered) return { detected: true, recovery: true, level: sweepLow };
    } else {
      const recentHigh = Math.max(...highs.slice(0, -2));
      const sweepHigh = highs[highs.length - 2];
      const recovered = closes[closes.length - 1] < opens[opens.length - 1] && closes[closes.length - 1] < recentHigh;
      if (sweepHigh > recentHigh * 1.002 && recovered) return { detected: true, recovery: true, level: sweepHigh };
    }
  ...
  checkDivergence(klines, direction) {
    ...
    if (direction === 'LONG') {
      ...
      if (priceLow < prevPriceLow && rsiLow > prevRsiLow) return { detected: true, type: 'bullish' };
    } else {
      ...
      if (priceHigh > prevPriceHigh && rsiHigh < prevRsiHigh) return { detected: true, type: 'bearish' };
    }
  ...
  suggestEntryPrice(klines, direction, entryType) {
    ...
    if (entryType === 'RETEST') {
      const highs = klines.map(k => parseFloat(k[2]));
      return Math.max(...highs.slice(-20, -10));
    }
    return lastClose;
  }
...
class ConfirmationLayers {
  ...
  async verify(symbol, price, direction, klines, funding, lsRatio, volumeSpike) {
    ...
    results.HTF_TREND = { passed: direction === 'LONG' ? htf.bullish : htf.bearish, score: direction === 'LONG' ? (htf.bullish ? 25 : 0) : (htf.bearish ? 25 : 0) };
    ...
    results.PRICE_ACTION = { passed: direction === 'LONG' ? pa.bullish : pa.bearish, score: direction === 'LONG' ? (pa.bullish ? 20 : 0) : (pa.bearish ? 20 : 0) };
    ...
    results.VOLUME_CONFIRMATION = { passed: volumeOk, score: volumeOk ? 15 : 0, details: `${volumeSpike.toFixed(1)}x avg` };
    ...
    results.ORDER_FLOW = { passed: direction === 'LONG' ? orderFlow.bullish : orderFlow.bearish, score: direction === 'LONG' ? (orderFlow.bullish ? 10 : 0) : (orderFlow.bearish ? 10 : 0), details: `imbalance ${orderFlow.imbalance || 0}` };
    totalScore += results.ORDER_FLOW.score;
    maxScore += 10;

    const liquidity = timing.checkLiquiditySweepPattern(klines, direction);
    results.LIQUIDITY = { passed: liquidity.detected && liquidity.recovery, score: (liquidity.detected && liquidity.recovery) ? 10 : 0, details: liquidity.detected ? 'sweep+recovery' : 'none' };
    totalScore += results.LIQUIDITY.score;
    maxScore += 10;

    const sentiment = analyzer.checkSentiment(funding, lsRatio);
    results.SENTIMENT = { passed: direction === 'LONG' ? sentiment.bullish : sentiment.bearish, score: direction === 'LONG' ? (sentiment.bullish ? 10 : 0) : (sentiment.bearish ? 10 : 0), details: `funding ${funding.toFixed(3)}% ls ${lsRatio.toFixed(2)}` };
    totalScore += results.SENTIMENT.score;
    maxScore += 10;

    const entryTiming = await timing.analyze(symbol, price, direction, klines, volumeSpike);
    results.TIMING = { passed: entryTiming.ready, score: entryTiming.ready ? 10 : 0, details: entryTiming.type || 'waiting', entryPrice: entryTiming.entryPrice };
    ...
    return { passed, score: parseFloat(overallScore.toFixed(1)), layers: results, failedRequired, entryPrice: results.TIMING.entryPrice };
  }
}

// ─────────────────────────────────────────────────────────────────────────────
// FIX #5: MARKET REGIME CLASSIFIER
// ─────────────────────────────────────────────────────────────────────────────
class MarketRegimeClassifier {
  constructor() {
    this.regimes = {
      ...
      SILENT: { name: 'SILENT', riskMultiplier: 0, minScore: 999 }
    };
  }
  ...
  async classify(btcData, altsData) {
    ...
    for (const regime of Object.values(this.regimes)) {
      if (btcRegime.riskMultiplier < finalRegime.riskMultiplier) finalRegime = regime;
      if (marketRegime.riskMultiplier < finalRegime.riskMultiplier) finalRegime = regime;
    }
  ...
  async analyzeBTC(btcData) {
    ...
    if (!klines || klines.length < 50) return { name: 'SILENT', riskMultiplier: 0, score: 30 };
    ...
    if (atrPct < 1.0) return { name: 'SILENT', riskMultiplier: 0, score: 30 };
    if (atrPct > 3.5) return { name: 'VOLATILE', riskMultiplier: 0.3, score: 60 };
    if (adx > 25) return { name: 'TRENDING', riskMultiplier: 1.0, score: 85 };
    return { name: 'RANGING', riskMultiplier: 0.5, score: 70 };
  }

  async analyzeMarket(altsData) {
    if (!altsData || altsData.length === 0) return { name: 'TRENDING', riskMultiplier: 1.0, score: 75 };
    let bullishCount = 0, totalVol = 0;
    ...
    if (breadth > 70) return { name: 'TRENDING', riskMultiplier: 1.0, score: 85 };
    if (breadth < 30) return { name: 'TRENDING', riskMultiplier: 0.8, score: 80 };
    if (avgVol < 1000000) return { name: 'SILENT', riskMultiplier: 0, score: 40 };
    return { name: 'RANGING', riskMultiplier: 0.5, score: 70 };
  }
}
...
class PositionManager {
  ...
  calculatePositionSize(accountBalance, entryPrice, stopLoss, confidence, regimeMultiplier) {
    ...
    positionSize = positionSize * confidenceMultiplier * regimeMultiplier;
    const maxSize = accountBalance * (RISK_CONFIG.max_position_size_pct / 100);
    return parseFloat(Math.min(positionSize, maxSize).toFixed(2));
  }
  ...
  async openPosition(symbol, direction, entryPrice, stopLoss, tp1, tp2, confidence, accountBalance, regimeMultiplier) {
    ...
    const position = {
      id: `${symbol}_${Date.now()}`, symbol, direction, entryPrice, stopLoss, tp1, tp2,
      size: positionSize, riskAmount, confidence, openTime: Date.now(), status: 'OPEN',
      trailingActivated: false, highestPrice: entryPrice, lowestPrice: entryPrice
    };
  ...
  async updatePositions(currentPrices) {
    ...
    for (const pos of this.positions) {
      ...
      let shouldClose = false, closeReason = '', exitPrice = currentPrice;
      ...
      if ((isLong && currentPrice >= pos.tp1) || (!isLong && currentPrice <= pos.tp1)) {
        shouldClose = true; closeReason = 'TP1_HIT'; exitPrice = pos.tp1;
      } else if ((isLong && currentPrice <= pos.stopLoss) || (!isLong && currentPrice >= pos.stopLoss)) {
        shouldClose = true; closeReason = 'SL_HIT'; exitPrice = pos.stopLoss;
      }
      
      if (!shouldClose && RISK_CONFIG.trailing_activation_pct > 0) {
        const profitPct = isLong ? ((currentPrice - pos.entryPrice) / pos.entryPrice) * 100 : ((pos.entryPrice - currentPrice) / pos.entryPrice) * 100;
        if (profitPct >= RISK_CONFIG.trailing_activation_pct) {
          if (!pos.trailingActivated) pos.trailingActivated = true;
          else {
            const trailStop = isLong ? pos.highestPrice * (1 - RISK_CONFIG.trailing_distance_pct / 100) : pos.lowestPrice * (1 + RISK_CONFIG.trailing_distance_pct / 100);
            if ((isLong && currentPrice <= trailStop) || (!isLong && currentPrice >= trailStop)) {
              shouldClose = true; closeReason = 'TRAILING_STOP'; exitPrice = currentPrice;
            }
          }
        }
      }
      ...
      if (!shouldClose && holdHours >= RISK_CONFIG.max_hold_hours) { shouldClose = true; closeReason = 'TIMEOUT'; }
      
      if (shouldClose) {
        const pnl = isLong ? (exitPrice - pos.entryPrice) * pos.size : (pos.entryPrice - exitPrice) * pos.size;
        ...
        if (pnl > 0) { account.consecutive_losses = 0; if (account.balance > account.peak_balance) account.peak_balance = account.balance; }
        else account.consecutive_losses++;
        const trade = { ...pos, exitPrice, pnl, pnlPct: parseFloat(pnlPct.toFixed(2)), closeReason, closeTime: Date.now() };
        account.trade_history.push(trade);
        closedPositions.push(trade);
        if (closeReason === 'TP1_HIT') await tg(OWNER_CHAT_ID, `✅ ${pos.symbol.replace('USDT', '')} TP1 HIT\nProfit: +${pnlPct.toFixed(1)}% | +$${pnl.toFixed(2)}`);
        else if (closeReason === 'SL_HIT') await tg(OWNER_CHAT_ID, `❌ ${pos.symbol.replace('USDT', '')} SL HIT\nLoss: ${pnlPct.toFixed(1)}% | -$${Math.abs(pnl).toFixed(2)}`);
      }
    }
  ...
  getOpenPositions() { return this.positions; }
}
...
// ─────────────────────────────────────────────────────────────────────────────
class RiskMonitor {
  constructor() { this.resetDailyIfNeeded(); this.resetWeeklyIfNeeded(); }
  resetDailyIfNeeded() {
    const today = new Date().toDateString();
    if (account.daily_date !== today) { account.daily_pnl = 0; account.daily_date = today; account.consecutive_losses = 0; }
  }
  resetWeeklyIfNeeded() {
    const weekStart = getWeekStart();
    if (account.weekly_date !== weekStart) { account.weekly_pnl = 0; account.weekly_date = weekStart; }
  }
  canTrade() {
    this.resetDailyIfNeeded(); this.resetWeeklyIfNeeded();
    ...
    if (dailyLossPct >= RISK_CONFIG.max_daily_loss_pct) return { allowed: false, reason: `Daily loss limit reached (${dailyLossPct.toFixed(1)}% / ${RISK_CONFIG.max_daily_loss_pct}%)` };
    if (weeklyLossPct >= RISK_CONFIG.max_weekly_loss_pct) return { allowed: false, reason: `Weekly loss limit reached (${weeklyLossPct.toFixed(1)}% / ${RISK_CONFIG.max_weekly_loss_pct}%)` };
    if (account.consecutive_losses >= RISK_CONFIG.max_consecutive_losses) return { allowed: false, reason: `${account.consecutive_losses} consecutive losses - cooling down` };
    if (account.open_positions.length >= RISK_CONFIG.max_concurrent_positions) return { allowed: false, reason: `Max concurrent positions (${RISK_CONFIG.max_concurrent_positions})` };
...
const checkBTCGate = async () => {
  try {
    ...
    const ema20 = new DirectionAnalyzer().calcEMA(closes, 20);
    const ema50 = new DirectionAnalyzer().calcEMA(closes, 50);
    ...
    return { pass, reason, price, change: change24h, change1h, funding: fundRate, emoji, trend };
  } catch { return { pass: true, reason: '⚠️ BTC data unavailable', price: 0, change: 0 }; }
};
...
const runFullMarketScan = async () => {
  ...
  try {
    ...
    const altsData = valid.map(t => ({ symbol: t.symbol, change24h: parseFloat(t.priceChangePercent), volume: parseFloat(t.quoteVolume) }));
    ...
    log(`📈 Market Regime: ${regime.regime} | Risk: ${regime.riskMultiplier}x`);
...
const runWatchlistScan = async () => {
  ...
  try {
    ...
    if (!riskCheck.allowed) { log(`⛔ ${riskCheck.reason}`); return; }
    ...
    for (const coin of topCoins) {
      ...
      try { klines = await fetchJSON(`https://fapi.binance.com/fapi/v1/klines?symbol=${symbol}&interval=15m&limit=50`); } catch { }
      ...
      const confirmation = await confirmationLayers.verify(symbol, price, directionResult.direction, klines, funding, ls, volumeSpike);
      if (!confirmation.passed) { log(`⏳ ${symbol} ${directionResult.direction} - confirmation score ${confirmation.score}% (need 70%)`); continue; }
      ...
      if (!regime.allowTrading) { log(`🌙 ${symbol} - market regime ${regime.regime} blocks trading`); continue; }
      ...
      if (existingPosition) { log(`📌 ${symbol} - position already open`); continue; }
      ...
      if (canAlert(signalKey)) {
        ...
        markAlert(signalKey);
        await positionManager.openPosition(symbol, directionResult.direction, price, sl, tp1, tp2, directionResult.confidence, account.balance, regime.riskMultiplier);
        alertsFired++;
    ...
    for (const pos of positionManager.getOpenPositions()) {
      try { const ticker = await fetchJSON(`https://fapi.binance.com/fapi/v1/ticker/price?symbol=${pos.symbol}`); currentPrices[pos.symbol] = parseFloat(ticker.price); } catch { }
    }
    const closed = await positionManager.updatePositions(currentPrices);
    if (closed.length) log(`📊 Closed ${closed.length} positions`);
    
    // FIXED: Get current regime for status update (regime was out of scope)
    let currentRegime = { regime: 'UNKNOWN', riskMultiplier: 1.0 };
    try {
      const btcKlinesForRegime = await fetchJSON('https://fapi.binance.com/fapi/v1/klines?symbol=BTCUSDT&interval=1h&limit=100');
      if (btcKlinesForRegime) {
        const btcRanges = btcKlinesForRegime.map(k => parseFloat(k[2]) - parseFloat(k[3]));
        const btcAvgRange = btcRanges.slice(-20).reduce((a, b) => a + b, 0) / 20;
        const btcData = { klines: btcKlinesForRegime, change24h: btc.change, atr: btcAvgRange };
        const marketRegime = await regimeClassifier.classify(btcData, []);
        currentRegime = marketRegime;
      }
    } catch (err) { log('Regime fetch error:', err.message); }
    
    const openCount = positionManager.getOpenPositions().length;
    await tg(OWNER_CHAT_ID, `👁 Scan #${watchlistScanCount} | ${gstNow()}\nOpen: ${openCount} | Today: ${account.daily_pnl > 0 ? '+' : ''}$${account.daily_pnl.toFixed(2)}\nBalance: $${account.balance.toFixed(2)} | Peak: $${account.peak_balance.toFixed(2)}\nRegime: ${currentRegime.regime} | ${btc.emoji} BTC ${btc.change > 0 ? '+' : ''}${btc.change?.toFixed(1)}%`);
...
const handleCommand = async msg => {
  ...
  if (text === '/start') {
    ...
    if (!positions.length) { await tg(chatId, '📭 No open positions'); return; }
    ...
    account = {
      balance: 10000, peak_balance: 10000, daily_pnl: 0, weekly_pnl: 0,
      daily_date: new Date().toDateString(), weekly_date: getWeekStart(),
      consecutive_losses: 0, open_positions: [], trade_history: [],
    };
