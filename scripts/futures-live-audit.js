...
const cfg = {
  ...
  assumedOrderNotionalUsd: 1_000,
  minEntryDepthImbalance: 0.75,
  minBidDepthRetention: 0.65,
  maxEntrySpreadExpansion: 1.75,
...
const benignContext = features => ({
  ...
  depth: {
    ...
    top3Imbalance: 1,
    assumedOrderNotionalUsd: 1_000,
    estimatedBuyPrice: features.last.close,
    entryImpactBps: 0,
    bidRetention: 1,
    spreadExpansion: 1,
  },
...
const rows = await mapLimit(universe, 6, async item => {
  ...
  if (features.impulse) {
    ...
    const depth = depthMetrics(book, features.last.close, cfg.assumedOrderNotionalUsd);
